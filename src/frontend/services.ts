...
export async function moveServerVolumeApi(serverId: string, newVolume: string, token: string | null): Promise<{ success: boolean; error?: string }> {
  const response = await fetch(`${API_BASE}/api/servers/${serverId}/move-volume`, {
    method: 'POST',
    headers: { ...getAuthHeaders(token), 'Content-Type': 'application/json' },
...
export async function exportIndicatorsCsvApi(serverIds: string[], token: string | null): Promise<void> {
  try {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `indicators-${new Date().toISOString().slice(0, 10)}.xlsx`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);
...
export async function generateLanguageReportApi(token: string | null): Promise<LanguageReportStats | null> {
  try {
    ...
    const blob = new Blob(['\uFEFF' + data.fourColumn], { type: 'text/csv;charset=utf-8' });
