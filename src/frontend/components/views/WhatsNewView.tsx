...
function PostEngagement(p: { rows: WhatsNewEventRow[] }) {
  ...
  const funnel = () => {
    ...
    for (const row of p.rows) sets[row.event].add(`${row.serverId}:${row.userEmail}`);
    return { seen: sets.seen.size, skipped: sets.skipped.size, completed: sets.completed.size };
  };

  const completionRate = () => {
    const f = funnel();
    return f.seen > 0 ? Math.round((f.completed / f.seen) * 100) : null;
  };
