...
import { WhatsNewStatsModal } from '../modals/WhatsNewStatsModal.tsx';
import type { WhatsNewFleetAverages } from '../modals/WhatsNewStatsModal.tsx';
import { PostEngagement, funnelCounts } from './WhatsNewEngagement.tsx';
...
export function WhatsNewView(props: WhatsNewViewProps) {
  ...
  const [statsPostId, setStatsPostId] = createSignal<string | null>(null);
  ...
  const postRows = (postId: string) => eventRows().filter(r => r.postId === postId);
  const postStats = (postId: string) => funnelCounts(postRows(postId));

  const statsPost = () => posts().find(p => p.id === statsPostId()) ?? null;

  // Mean per-post stats over every post with at least one event, so a post can
  // be judged against how announcements typically perform
  const fleetAverages = (): WhatsNewFleetAverages | null => {
    const perPost = posts()
      .map(post => funnelCounts(postRows(post.id)))
      .filter(c => c.seen + c.skipped + c.completed > 0);
    if (perPost.length === 0) return null;
    const mean = (xs: number[]) => xs.reduce((a, b) => a + b, 0) / xs.length;
    const rates = perPost.filter(c => c.seen > 0).map(c => (c.completed / c.seen) * 100);
    return {
      posts: perPost.length,
      seen: Math.round(mean(perPost.map(c => c.seen))),
      completed: Math.round(mean(perPost.map(c => c.completed))),
      skipped: Math.round(mean(perPost.map(c => c.skipped))),
      completionRate: rates.length > 0 ? Math.round(mean(rates)) : null,
    };
  };
            ...
            <div class="whats-new-list">
              ...
              <For each={posts()}>
                {(post) => {
                  ...
                  // A div, not a button: the Stats button nests inside it and
              ...
              {(d) => (
                <div class="whats-new-editor">
                        ...
                        <div class="whats-new-page-card whats-new-page-fields">
                          <div class="whats-new-page-header">
                            ...
                            <div class="whats-new-page-actions">
                              <button class="system-btn whats-new-btn-sm" title="Move this page earlier" disabled={activePage() === 0 || uploadingPage() !== null} onClick={() => movePage(activePage(), -1)}>Move ◀</button>
                              <button class="system-btn whats-new-btn-sm" title="Move this page later" disabled={activePage() === d().pages.length - 1 || uploadingPage() !== null} onClick={() => movePage(activePage(), 1)}>Move ▶</button>
                  ...
                  <Show when={selectedId() !== 'new'}>
                    <div class="whats-new-section-label whats-new-section-gap">Engagement</div>
                    <PostEngagement rows={eventRows().filter(r => r.postId === selectedId())} />
                  </Show>
